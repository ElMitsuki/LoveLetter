...
from QlearningEngineV1.node import Node
from QlearningEngineV1.Qfunction import setNodeExpectation, updateQTable

class AIV1(Player):

    def __init__(self, id, exploration_coeff):
        super().__init__(id, "IAv1")
        ...
        if have_princess:
            ...
            for card in self._hand:
                ...
                if(self.__previous_node != None):
                    node = Node(self.__previous_node.getCurrentState().getCard1(), self.__previous_node.getCurrentState().getCard2(), self.__previous_node.getCurrentAction(), self.__previous_node.getCurrentExpectation(), self._hand[0].getValue(), self._hand[1].getValue(), card.getValue())
                else:
                    node = Node(None, None, None, None, self._hand[0].getValue(), self._hand[1].getValue(), card.getValue())
            ...
            if self.__exploration_coeff > random.random():
                choosen_card = random.randint(0,1)
            elif choosen_card == -1:
                for node in nodes:
            ...
            if choosen_node is None:
                if len(nodes) == 1:
                    ...
                    choosen_node = nodes[0] if nodes[0].getCurrentAction() == self._hand[choosen_card] else nodes[1]
            if(self.__previous_node != None):
                pass#updateQTable(choosen_node, 0)
            self.__previous_node = choosen_node
            if self._hand[choosen_card] == 1 and self.__special == None:
                best_proba = 0
                card_value = 1
                equality = True
                ...
                if equality :
                    self.__special = value_remaining_cards[random.randint(0, len(value_remaining_cards)-1)]
                if best_proba == 0:
                    self.__special = 2
                
            elif self._hand[choosen_card] == 5 and self.__special == None:
                self.__special = random.randint(1,2)
        time.sleep(1)
